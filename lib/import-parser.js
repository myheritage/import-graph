...
const DYNAMIC_IMPORT_REGEXP     = /import\((['"`][^'"`]+['"`])\)/g;
...
class ImportParser {
     ...
     */
    parse(content) {
        let syntaxRegExps = this.syntaxRegExps,
            syntaxMatch = {},
            innerQuoteMatch = {},
            results = [],
        ...
        syntaxRegExps.forEach(syntaxRegExp => {
    ...
    /**
     * @name ImportParser.syntaxRegExps
     * @returns {[RegExp]}
     */
    get syntaxRegExps () {
        let returnedSyntaxRegExp = [ES6_IMPORT_REGEXP, DYNAMIC_IMPORT_REGEXP, COMMONJS_REQUIRE_REGEXP];

        if (typeof this.dependencyPattern === 'string') {
            switch (this.dependencyPattern) {
                case ES6_IMPORT_NAME:
                    returnedSyntaxRegExp = [ES6_IMPORT_REGEXP, DYNAMIC_IMPORT_REGEXP];
                    break;
